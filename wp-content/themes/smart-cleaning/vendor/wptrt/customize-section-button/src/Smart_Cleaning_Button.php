...
class Smart_Cleaning_Button extends WP_Customize_Section {
	...
	public function json() {
		...
		if ( ! $this->button_url && $smart_cleaning_theme->get( 'ThemeURI' ) ) {

			$button_url = $smart_cleaning_theme->get( 'ThemeURI' );

		// Fall back to the `Author URI` defined in `style.css`.
		} elseif ( ! $this->button_url && $smart_cleaning_theme->get( 'AuthorURI' ) ) {

			$button_url = $smart_cleaning_theme->get( 'AuthorURI' );
		}

		$json['button_text'] = $this->button_text ? $this->button_text : $smart_cleaning_theme->get( 'Name' );
